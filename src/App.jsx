...
import { fetchDefectos, upsertDefecto, deleteDefecto, bulkUpsertDefectos, saveGiro, fetchGiros, fetchGiro, deleteGiro, updateGiroRows, savePdca, fetchPdcas, saveUnificacion, signIn, signOut, getSession, onAuthChange } from './supabase';
...
export default function App() {
  ...
  // Load defectos when authenticated
  useEffect(() => {
    if (session) fetchDefectos().then(setDefectos).catch(e => console.error(e));
  }, [session]);
  ...
  // Occurrence map from current giro
  const occurrenceMap = useMemo(() => {
    if (!result) return {};
    const m = {};
    for (const r of result.qaRows) {
      m[r.defectName] = (m[r.defectName] || 0) + r.cantDefectos;
    }
    return m;
  }, [result]);
  ...
  const handleLogin = useCallback(async (e) => {
    ...
    try { await signIn(loginEmail, loginPass); } catch (err) { setAuthError(err.message || 'Credenciales incorrectas'); }
  ...
  const handleProcess = useCallback(async () => {
    ...
    try {
      ...
      try { const saved = await saveGiro({ ...res, name, date: new Date().toISOString().split('T')[0] }); if (saved?.id) { setGiroId(saved.id); const pd = await fetchPdcas(saved.id); setPdcaMap(pd); } } catch (e) { console.warn(e); }
      setPage('matrix');
  ...
  const handleUnify = useCallback(async (destNum, origenInput) => {
    ...
    if (giroId) { try { await updateGiroRows(giroId, newRows, newSummary); for (const o of origenNums) await saveUnificacion(giroId, destNum, o); } catch (e) { console.warn(e); } }
  ...
  const handleUploadDefectos = useCallback(async (file) => {
    try {
      ...
      for (let i = 1; i < rows.length; i++) {
        const r = rows[i]; const nombre = r[0]; if (!nombre || typeof nombre !== 'string') continue;
        toUpsert.push({ nombre: String(nombre).trim(), severidad: parseInt(r[1]) || 3, costo_interno: parseInt(r[2]) || 1, costo_externo: parseInt(r[3]) || 4 });
      }
      if (toUpsert.length === 0) { alert('No se encontraron defectos'); return; }
  ...
  // Download defectos as Excel
  const handleDownloadDefectos = useCallback(() => {
    // Build CSV with BOM for Excel compatibility
    const bom = '\uFEFF';
    const header = 'Nombre,Severidad,Costo Interno,Costo Externo,Ocurrencia\n';
    const rows = defectos.map(d => {
      const occ = occurrenceMap[d.nombre] || 0;
      return `"${d.nombre}",${d.severidad},${d.costo_interno},${d.costo_externo},${occ}`;
    }).join('\n');
    const blob = new Blob([bom + header + rows], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = `defectos_QA_${new Date().toISOString().split('T')[0]}.csv`; a.click();
    URL.revokeObjectURL(url);
  }, [defectos, occurrenceMap]);
  ...
  const handleDeleteGiro = useCallback(async (id, e) => {
    e.stopPropagation();
    if (!confirm('¿Eliminar este giro? Esta acción no se puede deshacer.')) return;
    try { await deleteGiro(id); setGiros(prev => prev.filter(g => g.id !== id)); } catch (err) { alert('Error: ' + err.message); }
  }, []);
      ...
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(165deg,#0F172A,#1E293B 50%,#0F172A)', padding: 24 }}>
        ...
        <form onSubmit={handleLogin} style={{ width: '100%', maxWidth: 400, background: 'rgba(30,41,59,0.8)', borderRadius: 16, padding: 32, border: '1px solid #334155' }}>
          ...
          <label style={{ display: 'block', marginBottom: 16 }}>
            <span style={{ fontSize: 12, fontWeight: 600, color: '#94A3B8', display: 'block', marginBottom: 6 }}>Email</span>
          ...
          <label style={{ display: 'block', marginBottom: 24 }}>
            <span style={{ fontSize: 12, fontWeight: 600, color: '#94A3B8', display: 'block', marginBottom: 6 }}>Contraseña</span>
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(165deg,#0F172A,#1E293B 50%,#0F172A)', padding: 24 }}>
        <div style={{ position: 'absolute', top: 20, right: 20, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <Btn onClick={handleLogout} style={{ padding: '5px 12px', fontSize: 11 }}>Salir</Btn>
        </div>
      ...
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(165deg,#0F172A,#1E293B 50%,#0F172A)', padding: 24 }}>
        ...
        {!pendingFile ? (
          <div onDragOver={e => { e.preventDefault(); e.stopPropagation(); setDragOver(true); }} onDragLeave={e => { e.preventDefault(); setDragOver(false); }}
            ...
            <p style={{ fontSize: 14, color: '#64748B', margin: '0 0 16px' }}>SurveyMonkey (.xlsx)</p>
      ...
      <div style={{ minHeight: '100vh', padding: 24, maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <Btn onClick={handleDownloadDefectos} bg="#16A34A" color="#fff">📥 Descargar</Btn>
            <input ref={defFileRef} type="file" accept=".xlsx,.xls,.csv" onChange={e => { if (e.target.files[0]) handleUploadDefectos(e.target.files[0]); e.target.value = ''; }} style={{ display: 'none' }} />
          </div>
        </div>
        <p style={{ fontSize: 12, color: '#64748B', marginBottom: 16 }}>Excel para cargar: Columna A = Nombre, B = Severidad, C = Costo Interno, D = Costo Externo (fila 1 = encabezado). La columna Ocurrencia se muestra solo si hay un giro activo.</p>
        ...
        <div style={{ overflowX: 'auto', borderRadius: 12, border: '1px solid #334155' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr style={{ background: '#1E293B' }}>
              <th style={{ ...th, textAlign: 'left', minWidth: 280 }}>Defecto</th><th style={th}>Severidad</th><th style={th}>C.Int</th><th style={th}>C.Ext</th><th style={th}>Ocurrencia</th><th style={th}>Acciones</th>
            </tr></thead>
            <tbody>
              {filtered.map((d, i) => {
                const occ = occurrenceMap[d.nombre] || 0;
                return (
                  <tr key={d.id} style={{ background: i % 2 === 0 ? '#0F172A' : '#131C2E', borderBottom: '1px solid #1E293B' }}>
                    ...
                    <td style={{ ...td, fontWeight: occ > 0 ? 700 : 400, color: occ > 0 ? '#F59E0B' : '#475569' }}>{occ}</td>
                    <td style={td}>
            ...
            {giros.map(g => (
              <div key={g.id} onClick={() => loadGiro(g.id)} style={{ background: '#1E293B', borderRadius: 12, padding: '16px 20px', border: '1px solid #334155', cursor: 'pointer', transition: 'border-color .2s' }}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <div style={{ fontWeight: 600, color: '#F8FAFC', fontSize: 16 }}>{g.name}</div>
                    <div style={{ color: '#64748B', fontSize: 12, marginTop: 2 }}>{g.date} · {g.bancos_controlados?.toLocaleString()} bancos · {g.total_defects} defectos</div>
                  </div>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    ...
                    <button onClick={(e) => handleDeleteGiro(g.id, e)}
                      style={{ padding: '6px 10px', background: '#7F1D1D', color: '#FCA5A5', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 12, fontWeight: 600, marginLeft: 8 }}>🗑️</button>
                  </div>
                </div>
              </div>
            ))}
                  ...
                  <tr key={row.vozNum} onClick={() => setSelectedRow(sel ? null : row.vozNum)}
                    ...
                    <td style={{ ...td, borderLeft: '2px solid #334155' }} onClick={e => e.stopPropagation()}>
                      <div style={{ display: 'flex', gap: 2, justifyContent: 'center' }}>
                        {['P', 'D', 'C', 'A'].map((l, li) => { const f = ['plan', 'do_step', 'check', 'act'][li]; const ck = pc[f]; return (<button key={l} onClick={() => handlePdca(row.vozNum, f, !ck)} style={{ width: 20, height: 20, borderRadius: 3, border: 'none', fontSize: 9, fontWeight: 700, cursor: 'pointer', background: ck ? '#16A34A' : '#334155', color: ck ? '#fff' : '#64748B' }}>{l}</button>); })}
                      </div>
                    </td>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                        <div>
                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(120px,1fr))', gap: 8, marginBottom: 12 }}>
                            <Dt l="Componente" v={row.component} /><Dt l="Ocurrencia %" v={`${(row.ocurrenciaPct * 100).toFixed(4)}%`} m />
                          ...
                          <div style={{ marginTop: 8, padding: '10px 12px', background: '#0F172A', borderRadius: 8, border: '1px solid #334155' }}>
                            ...
                            <p style={{ fontSize: 10, color: '#64748B', marginTop: 4 }}>Las ocurrencias se suman y las voces indicadas se eliminan</p>
                          </div>
                        </div>
                        <div>
